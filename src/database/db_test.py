...
class TestDB(unittest.TestCase):
    ...
    def setUp(self): # test connection to database
        try:
    ...
    def test_insertion(self): # test insertion, 1 for inserting, 0 for dup
        ...
        post = db.insertion(self.collection, url, freq, last_updated, response)
    ...
    def test_scanner(self): # test scanner by inserting expired link and find if scanner returns that
        url = "www.yahoo.com"
        freq = 7
        last_updated = datetime.today() - timedelta(days = 14)
        response = True

        db.insertion(self.collection, url, freq, last_updated, response)        
